...
.deal { font-size:10px; margin-top:3px; padding:1px 6px; border-radius:8px; display:inline-block; font-weight:600; }
.deal.buy { background:#0b3b2e; color:#86efac; border:1px solid #14532d; }
.deal.mid { background:#3b3410; color:#fde68a; border:1px solid #78550a; }
.deal.high { background:#3b1e1e; color:#fca5a5; border:1px solid #7f1d1d; }
.combo .deal { font-size:11px; margin-left:8px; vertical-align:middle; }
...
def _deal_badge(frac, n):
    """Selo 'vale a pena?' — quão barato o preço atual está vs. os últimos 30 dias."""
    if frac is None or n < 15:
        return ""  # histórico insuficiente p/ opinar
    pct = round(frac * 100)
    if frac >= 0.7:
        cls, ico = "buy", "🟢 ótimo"
    elif frac >= 0.4:
        ...
        cls, ico = "high", "🔴 caro"
    return f'<div class="deal {cls}">{ico} · + barato que {pct}% (30d)</div>'
...
def _leg_table(db, frm, to, title):
    ...
    for r in rows:
        ...
        deal = _deal_badge(*db.deal_score(frm, to, r["date"], "oneway", p)) if p is not None else ""
        ...
        trs.append(
            ...
            f'<td class="price">{_money(p)}{min_txt}{deal}</td>'
...
def render_dashboard(db, origin, dest, routes=None, msg=None):
    ...
    if ida and volta:
        ...
        di = _deal_badge(*db.deal_score(origin, dest, bi["date"], "oneway", bi["price"])) if bi["price"] else ""
        dv = _deal_badge(*db.deal_score(dest, origin, bv["date"], "oneway", bv["price"])) if bv["price"] else ""
        combo = (
...
def render_roundtrip(db, origin, dest):
    ...
    rdeal = _deal_badge(*db.deal_score(origin, dest, best["departure_date"], "roundtrip",
                                       best["price"], best["return_date"])) if best["price"] else ""
    card = ('<div class="combo"><div class="lab">Melhor ida-e-volta casada agora</div>'
            f'<div class="big">{_money(best["price"])}{rdeal}</div>'
